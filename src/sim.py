...
class Body:
    ...
    def __init__(self, mass = 1, radius = .05, position = (0, 0, 122), velocity = (0, 0, 0), angle = 0, angle_vel = 0, drag = (0,0,0), target = (0,0), xy_force = (0,0), angle_force = 0, pos_pid = (15, .15, 5), ang_pid = (2, .001, .2)):
        # constants
        self.mass = mass
        self.radius = radius
        self.drag = drag
        ...
        self.controller = Controller(self, pos_pid=pos_pid, ang_pid=ang_pid)
...
class Controller:
    ...
    def __init__(self, body:Body, pos_pid, ang_pid):
        self.body = body
        self.force_lim = 50
        self.angle_force_lim = 10
        
        # xy PID constants
        self.p, self.i, self.d = pos_pid
    
        # angle PID constants
        self.ang_p, self.ang_i, self.ang_d = ang_pid
...
class Simulator:
    ...
    def __init__(self, body:Body, wind = (0,0,0), time_step = 0.01, debug = False):
        self.debug = debug
        ...
        # first number says if random wind is active
        self.wind = list(wind)
...
def net_accel(body:Body, sim:Simulator):
    ...
    # need to implement
    # takes fin deflection and angle to get x,y force
    x_force = body.xy_force[0] *  math.cos(body.angle/convert) + body.xy_force[1] *  math.sin(body.angle/convert)
    y_force = body.xy_force[1] *  math.cos(body.angle/convert) + body.xy_force[0] *  math.sin(body.angle/convert)

    if sim.wind[0] == 1:
        rand_wind_x = random.randint(0,10)
        rand_wind_y = random.randint(0,10)
    else:
        rand_wind_x = 0
        rand_wind_y = 0

    x_accel = body.drag[0] * ((body.velocity[0]) ** 2) + sim.wind[1] + rand_wind_x + x_force
    y_accel = body.drag[1] * ((body.velocity[1]) ** 2) + sim.wind[2] + rand_wind_y + y_force
...
def graph(body:Body, sim:Simulator):
...
def run_skydiver_simulation(animate_sim):
    ...
    if animate_sim:
        ...
        body = Body(mass=1, position=[0, 0, 122], velocity=[0, 0, 0], drag=(0, 0, 0), target=(-7,-15), angle = 30)
        sim = Simulator(body, wind = (0,0,0), debug=True)
        sim.run()
        graph(body,sim)
